...
- `data/prospects/*.json` を横断確認する
- `expanded-area` 系JSONを優先する
- 最新のリサーチレポートを確認する
- 条件を満たす候補を抽出する
- 10件未満ならWeb補助リサーチを行う
- 送信候補A、条件付き候補、新規候補、除外候補に分類する
- スプレッドシートの自動更新は行わない
...
1. `data/prospects/*.json` を横断確認
2. `expanded-area` 系JSONを優先
3. `docs/reports/sales/research/*.md` の直近14日以内のリサーチレポートを確認
4. 未検収/検収済で、連絡導線がある候補を優先
5. 10件未満なら対象地域内でWeb補助リサーチ
6. Web補助リサーチ候補は `新規候補` として記録し、スプレッドシートへ自動投入しない
...
- 返信あり・商談化候補を整理する
- 初回返信文、無料簡易SNS診断の方針、商談準備、次アクションを出す
- 人間が更新すべきステータスを提案する
