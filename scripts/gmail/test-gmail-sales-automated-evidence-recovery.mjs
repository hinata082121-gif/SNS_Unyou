...
function installSheets(context, sourceRows, reviewRows = []) {
  const headers = Array.from(new Set([
    ...
    'noApiDecisionReason', 'prospectId', 'dedupeKey', 'email', 'contactEmail', 'name', 'subject', 'body',
...
[
  ...
  'no_api_source_row_relink_by_stable_identity_v1',
...
assert.equal(noapi1Status.effectiveAiApiAllowed, false);
assert.equal(noapi1Status.effectiveGroundingAllowed, false);
assert.equal(noapi1Status.aiRecoverySuppressedByNoPaidPolicy, true);
assert.equal(noapi1Status.aiRecoverySuppressedReason, 'paid_ai_api_disabled_by_policy');
const noapi1AiConfig = noapi1.inspectGmailSalesAiProviderConfiguration();
assert.equal(noapi1AiConfig.providerConfigured, true);
assert.equal(noapi1AiConfig.apiKeyPresent, true);
assert.equal(noapi1AiConfig.configurationValid, true);
assert.equal(noapi1AiConfig.effectiveAiApiAllowed, false);
assert.equal(noapi1AiConfig.effectiveGroundingAllowed, false);
assert.equal(noapi1AiConfig.aiProviderConfiguredButPolicyDisabled, true);
assert.equal(noapi1AiConfig.aiProviderPolicyDisabledReason, 'no_paid_ai_until_first_revenue');
assert.equal(noapi1AiConfig.recommendedOperatorAction, 'do_not_run_ai_provider_steps');
assert.equal(noapi1AiConfig.aiApiCalled, false);
...
assert.equal(noapi3Inspect.effectiveAiApiAllowed, false);
assert.equal(noapi3Inspect.effectiveGroundingAllowed, false);
assert.equal(noapi3Inspect.aiRecoverySuppressedByNoPaidPolicy, true);
assert.equal(noapi3Inspect.aiRecoverySuppressedReason, 'paid_ai_api_disabled_by_policy');
assert.equal(noapi3Inspect.noApiInventoryExhausted, true);
assert.equal(noapi3Inspect.noApiRecoveredTotalReadyInventory, 0);
assert.equal(noapi3Inspect.noApiRemainingShortfallToExact30, 30);
assert.equal(noapi3Inspect.noApiNextSafePath, 'add_new_non_private_non_freemail_public_business_inventory_or_manual_exception_review');
assert.equal(noapi3Inspect.noApiUnsafePathsSuppressed.includes('paid_ai_api'), true);
assert.equal(noapi3Inspect.noApiUnsafePathsSuppressed.includes('gemini_grounding'), true);
assert.equal(noapi3Inspect.noApiUnsafePathsSuppressed.includes('urlfetch_grounding'), true);
assert.equal(noapi3Inspect.sendBlockedReason, 'exact30_not_satisfied');
assert.equal(noapi3Inspect.manifestBlockedReason, 'ready_inventory_below_exact30');
assert.equal(noapi3Inspect.expectedApiClass, 'none');
assert.equal(noapi3Inspect.expectedWriteClass, 'none');
assert.equal(noapi3Inspect.evidenceRecoveryAction, 'wait_for_no_api_recoverable_inventory_or_manual_exception_review');
assert.equal(noapi3Inspect.evidenceRecoveryActionReasonCode, 'no_api_recoverable_inventory_insufficient');
assert.equal(noapi3Inspect.actionBlockedReason, 'no_api_recoverable_inventory_insufficient');
assert.equal(noapi3Inspect.aiRetryRecommendedAction, 'wait_for_no_api_recoverable_inventory_or_manual_exception_review');
...
assert.equal(noapi3Status.noApiInventoryExhausted, true);
assert.equal(noapi3Status.sendBlockedReason, 'exact30_not_satisfied');
assert.equal(noapi3Status.manifestBlockedReason, 'ready_inventory_below_exact30');
...
assert.equal(noapi3Usage.noApiInventoryExhausted, true);
assert.equal(noapi3Usage.sendBlockedReason, 'exact30_not_satisfied');
assert.equal(noapi3Usage.manifestBlockedReason, 'ready_inventory_below_exact30');
...
setSheetValueByHeader(noapi6.__reviewSheet, 5, 'sourceRowDigest', '');
setSheetValueByHeader(noapi6.__reviewSheet, 5, 'leadIdHash', '');
...
setSheetValueByHeader(noapi7.__reviewSheet, 6, 'sourceRowDigest', '');
setSheetValueByHeader(noapi7.__reviewSheet, 6, 'leadIdHash', '');
...
noapi8References[9] = 'http://public-relink-one.example.invalid/contact';
noapi8References[10] = 'https://public-relink-two.example.invalid/inquiry';
...
for (let rowIndex = 13; rowIndex <= 31; rowIndex += 1) {
  const sourceReference = ['https:', '', `existing-${rowIndex}.example.invalid`, 'contact'].join('/');
  setSheetValueByHeader(noapi8.__sourceSheet, rowIndex, 'sourceReference', sourceReference);
  setSheetValueByHeader(noapi8.__sourceSheet, rowIndex, 'sourceReferenceHash', noapi8.buildGmailSalesSourceReferenceHash_('official_site', sourceReference));
}
...
const noapi8Status = noapi8.inspectGmailSalesNoApiRecoveryStatus({ skipLog: true });
assert.equal(noapi8Status.readyInventoryCount, 9);
assert.equal(noapi8Status.noApiSourceRowRelinkCandidateCount, 2);
assert.equal(noapi8Status.noApiSourceRowRelinkEligibleCount, 2);
assert.equal(noapi8Status.noApiSourceRowRelinkBlockedCount, 0);
assert.equal(noapi8Status.noApiRecoverableCandidateCount, 2);
assert.equal(noapi8Status.noApiValidBusinessContactExceptionRecoverableCount, 2);
assert.equal(noapi8Status.noApiProjectedReadyInventoryCount, 11);
assert.equal(noapi8Status.noApiShortfallToExact30, 19);
assert.equal(noapi8Status.plannedNextAction, 'no_api_legacy_evidence_recovery');
assert.equal(noapi8Status.plannedExpectedApiClass, 'none');
assert.equal(noapi8Status.plannedExpectedWriteClass, 'sheets_review_recovery_only');
assert.equal(noapi8Status.plannedSafeToExecute, true);
assert.equal(noapi8Status.operatorRecommendedNextFunction, 'runGmailSalesNoApiLegacyEvidenceRecoveryStepOnce');
...
assert.equal(noapi8Residual.noApiShortfallToExact30, 19);
assert.equal(noapi8Residual.noApiRecoverableCandidateCount, 2);
assert.equal(noapi8Residual.sourceRowNotFoundCount, 2);
assert.equal(noapi8Residual.sourceRowRelinkOpportunityCount, 2);
assert.equal(noapi8Residual.noApiSourceRowRelinkEligibilityRuleId, 'no_api_source_row_relink_by_stable_identity_v1');
assert.equal(noapi8Residual.noApiSourceRowRelinkWouldRecoverCount, 2);
assert.equal(noapi8Residual.noApiSourceRowRelinkStillShortfallToExact30, 19);
assert.equal(noapi8Residual.sourceRowRelinkEligibleSampleSanitized.length, 2);
...
assert.equal(noapi8Residual.unsupportedSchemeCount >= 2, true);
...
const noapi8Step = noapi8.runGmailSalesNoApiLegacyEvidenceRecoveryStepOnce();
assert.equal(noapi8Step.status, 'pass');
assert.equal(noapi8Step.noApiRecoveredCount, 2);
assert.equal(noapi8Step.noApiSourceRowRelinkRecoveredCount, 2);
assert.equal(noapi8Step.noApiRecoveredByBasis.valid_business_contact_exception, 2);
assert.equal(noapi8Step.noApiRecoveredByRuleId.no_api_source_row_relink_by_stable_identity_v1, 2);
assert.equal(noapi8Step.readyInventoryCountAfterRecovery, 11);
assert.equal(noapi8Step.noApiStillShortfallToExact30, 19);
assert.equal(noapi8Step.aiApiCalled, false);
assert.equal(noapi8Step.urlFetchExecuted, false);
assert.equal(noapi8Step.gmailSendExecuted, false);
assert.equal(noapi8Step.gmailDraftCreated, false);
assert.equal(noapi8Step.triggerChanged, false);
const noapi8RowsAfter = noapi8.readSheetObjects_(noapi8.__sourceSheet).items;
assert.equal(String(noapi8RowsAfter[9].row.contactBasisType || ''), 'valid_business_contact_exception');
assert.equal(String(noapi8RowsAfter[9].row.recoveredFrom || ''), 'source_row_relink_by_stable_identity');
assert.equal(String(noapi8RowsAfter[9].row.deterministicRuleId || ''), 'no_api_source_row_relink_by_stable_identity_v1');
assert.equal(String(noapi8RowsAfter[10].row.deterministicRuleId || ''), 'no_api_source_row_relink_by_stable_identity_v1');
const noapi8Usage = noapi8.inspectGmailSalesRecoveryUsageLedger();
assert.equal(noapi8Usage.noApiSourceRowRelinkCandidateCount, 2);
assert.equal(noapi8Usage.noApiSourceRowRelinkEligibleCount, 0);
assert.equal(noapi8Usage.noApiSourceRowRelinkRecoveredCountToday, 2);
assert.equal(noapi8Usage.noApiRecoveredByRuleId.no_api_source_row_relink_by_stable_identity_v1, 2);
assert.equal(noapi8.__state.urlFetchCount, 0);
assert.equal(noapi8.__state.gmailSendCount, 0);
assert.equal(noapi8.__state.draftCreateCount, 0);
assert.equal(noapi8.__state.triggerCreateCount, 0);

const noapi9 = createContext();
noapi9.__props.GMAIL_SALES_PAID_AI_API_DISABLED_BY_POLICY = 'true';
noapi9.__props.GMAIL_SALES_AI_PROVIDER = 'gemini';
noapi9.__props.GMAIL_SALES_AI_MODEL = 'gemini-2.5-flash-lite';
noapi9.__props.GMAIL_SALES_AI_API_KEY = 'x';
installFiveAiPendingRows(noapi9);
applyTodayTargetDate(noapi9);
seedUrlFetchAuthorizationVerified(noapi9);
noapi9.inspectGmailSalesContactBasisCoverage_ = () => ({
  event: 'gmail_sales_contact_basis_coverage',
  mode: 'read_only',
  sourceCandidateCount: 30,
  fieldsSupported: true,
  approvedBasisCount: 11,
  eligibleAfterBasisCheckCount: 11,
  operationalCandidateReady: false,
  blockedReasons: ['eligible_basis_count_below_30'],
  gmailSendExecuted: false,
  googleSheetsUpdated: false,
...
const noapi9Status = noapi9.inspectGmailSalesAutomatedEvidenceRecoveryStatus_({ skipLog: true });
assert.equal(noapi9Status.readyInventoryCount, 11);
assert.equal(noapi9Status.noApiRecoverableCandidateCount, 0);
assert.equal(noapi9Status.noApiProjectedReadyInventoryCount, 11);
assert.equal(noapi9Status.noApiShortfallToExact30, 19);
assert.equal(noapi9Status.exact30Satisfied, false);
assert.equal(noapi9Status.manifestReady, false);
assert.equal(noapi9Status.noApiInventoryExhausted, true);
assert.equal(noapi9Status.noApiRecoveredTotalReadyInventory, 11);
assert.equal(noapi9Status.noApiRemainingShortfallToExact30, 19);
assert.equal(noapi9Status.noApiNextSafePath, 'add_new_non_private_non_freemail_public_business_inventory_or_manual_exception_review');
assert.equal(noapi9Status.noApiUnsafePathsSuppressed.includes('paid_ai_api'), true);
assert.equal(noapi9Status.noApiUnsafePathsSuppressed.includes('gemini_grounding'), true);
assert.equal(noapi9Status.noApiUnsafePathsSuppressed.includes('urlfetch_grounding'), true);
assert.equal(noapi9Status.noApiUnsafePathsSuppressed.includes('private_personal_contact'), true);
assert.equal(noapi9Status.noApiUnsafePathsSuppressed.includes('freemail_contact'), true);
assert.equal(noapi9Status.noApiUnsafePathsSuppressed.includes('business_directory_default'), true);
assert.equal(noapi9Status.plannedNextAction, 'wait_for_no_api_recoverable_inventory_or_manual_exception_review');
assert.equal(noapi9Status.plannedNextActionReasonCode, 'no_api_recoverable_inventory_insufficient');
assert.equal(noapi9Status.plannedExpectedApiClass, 'none');
assert.equal(noapi9Status.plannedExpectedWriteClass, 'none');
assert.equal(noapi9Status.plannedSafeToExecute, false);
assert.equal(noapi9Status.operatorRecommendedNextFunction, '');
assert.equal(noapi9Status.operatorRecommendedNextFunctionReason, '');
assert.equal(noapi9Status.operatorShouldRunSafeStepNow, false);
assert.equal(noapi9Status.operatorShouldWaitReason, 'no_api_recoverable_inventory_insufficient');
assert.equal(noapi9Status.aiRetryRecommendedAction, 'wait_for_no_api_recoverable_inventory_or_manual_exception_review');
assert.equal(noapi9Status.aiRetrySafeToExecute, false);
assert.equal(noapi9Status.expectedApiClass, 'none');
assert.equal(noapi9Status.expectedWriteClass, 'none');
assert.equal(noapi9Status.evidenceRecoveryAction, 'wait_for_no_api_recoverable_inventory_or_manual_exception_review');
assert.equal(noapi9Status.evidenceRecoveryActionReasonCode, 'no_api_recoverable_inventory_insufficient');
assert.equal(noapi9Status.actionStatus, 'blocked');
assert.equal(noapi9Status.actionBlockedReason, 'no_api_recoverable_inventory_insufficient');
assert.equal(noapi9Status.actionEstimatedCostYen, 0);
assert.equal(noapi9Status.actionCostYen, 0);
assert.equal(noapi9Status.effectiveAiApiAllowed, false);
assert.equal(noapi9Status.effectiveGroundingAllowed, false);
assert.equal(noapi9Status.aiRecoverySuppressedByNoPaidPolicy, true);
assert.equal(noapi9Status.aiRecoverySuppressedReason, 'paid_ai_api_disabled_by_policy');
assert.equal(noapi9Status.sendBlockedReason, 'exact30_not_satisfied');
assert.equal(noapi9Status.manifestBlockedReason, 'ready_inventory_below_exact30');
assert.equal(noapi9Status.aiApiCalled, false);
assert.equal(noapi9Status.urlFetchExecuted, false);
assert.equal(noapi9.__state.urlFetchCount, 0);
assert.equal(noapi9.__state.gmailSendCount, 0);
assert.equal(noapi9.__state.draftCreateCount, 0);
assert.equal(noapi9.__state.triggerCreateCount, 0);
const noapi9NoApiStatus = noapi9.inspectGmailSalesNoApiRecoveryStatus({ skipLog: true });
assert.equal(noapi9NoApiStatus.noApiInventoryExhausted, true);
assert.equal(noapi9NoApiStatus.noApiRecoveredTotalReadyInventory, 11);
...
const importSourceRows = makeSourceRows(30).map((row, index) => Object.assign({}, row, {
  prospectId: `import-source-${index + 1}`,
  dedupeKey: `import-source-dedupe-${index + 1}`,
  email: `ready${index + 1}@ready-${index + 1}.example.invalid`,
  contactEmail: `ready${index + 1}@ready-${index + 1}.example.invalid`,
  subject: `masked subject ${index + 1}`,
...
const importReviewRows = importSourceRows.slice(0, 9).map((row) => {
  const sourceRowKey = import1.hashValue_(`stable|${row.prospectId || row.dedupeKey}`);
  return {
    sourceRowKey,
    sourceRowDigest: import1.computeGmailSalesStableSourceIdentityDigest_(row, { sourceRowKey }),
    aiEvidenceDigest: `fixture-${sourceRowKey}`,
    lastAiEvaluatedEvidenceDigest: `fixture-${sourceRowKey}`
  };
});
installSheets(import1, importSourceRows, importReviewRows);
import1.loadSuppressionLedgerFromProperties_ = () => ({ loaded: true, recipientHashes: {}, domainHashes: {}, businessFingerprints: {}, entries: [] });
const importSourceReadBack = import1.readSheetObjects_(import1.__sourceSheet).items[0].row;
const importReviewReadBack = import1.readSheetObjects_(import1.__reviewSheet).items[0].row;
assert.equal(import1.buildGmailSalesStrictContactSourceRowKey_(importSourceReadBack), importReviewReadBack.sourceRowKey);
assert.equal(import1.computeGmailSalesStableSourceIdentityDigest_(importSourceReadBack, { sourceRowKey: importReviewReadBack.sourceRowKey }), importReviewReadBack.sourceRowDigest);
const eligibleImportRows = Array.from({ length: 11 }, (_, index) => {
  const n = index + 1;
  return {
...
const blockedImportRows = Array.from({ length: 19 }, (_, index) => Object.assign({}, eligibleImportRows[index % eligibleImportRows.length], {
  targetDate: '2026-06-01',
  publicEmail: `ready${index + 1}@ready-${index + 1}.example.invalid`,
  officialWebsiteUrl: `https://ready-${index + 1}.example.invalid/`,
  sourceUrl: `https://ready-${index + 1}.example.invalid/contact`,
  importStatus: 'imported'
}));
...
assert.equal(importReadiness.readyInventoryCount, 30);
assert.equal(importReadiness.rawReadyInventoryCount, 30);
assert.equal(importReadiness.currentEligibleCount, 9);
assert.equal(importReadiness.currentEligibleShortfallCount, 21);
assert.equal(importReadiness.exactThirtyRequiredCount, 30);
assert.equal(importReadiness.shortfallToExact30, 21);
assert.equal(importReadiness.importSheetPresent, true);
assert.equal(importReadiness.importRowCount, 30);
assert.equal(importReadiness.importCandidateCount, 30);
assert.equal(importReadiness.importEligibleCount, 11);
assert.equal(importReadiness.importBlockedCount, 19);
assert.equal(importReadiness.importBlockedReasonCounts.already_imported, 19);
assert.equal(importReadiness.importBlockedReasonCounts.target_date_invalid, 19);
assert.equal(importReadiness.importBlockedReasonCounts.duplicate_identity, 19);
assert.equal(importReadiness.proposedImportCount, 11);
assert.equal(importReadiness.projectedPotentialEligibleCountAfterReview, 20);
assert.equal(importReadiness.projectedCurrentEligibleCountAfterImport, 9);
assert.equal(importReadiness.projectedCurrentEligibleShortfallAfterImport, 21);
assert.equal(importReadiness.exact30SatisfiedAfterProjectedImport, false);
...
const zeroShortfallReadiness = import1.buildGmailSalesPublicBusinessLeadImportReadinessResult_({
  currentEligibleCount: 30, currentEligibleShortfallCount: 0, importEligibleCount: 11, proposedImportCount: Math.min(11, 0, 30)
});
assert.equal(zeroShortfallReadiness.proposedImportCount, 0);
assert.equal(zeroShortfallReadiness.plannedSafeToExecute, false);
const zeroEligibleReadiness = import1.buildGmailSalesPublicBusinessLeadImportReadinessResult_({
  currentEligibleCount: 9, currentEligibleShortfallCount: 21, importEligibleCount: 0, proposedImportCount: Math.min(0, 21, 30)
});
assert.equal(zeroEligibleReadiness.proposedImportCount, 0);
assert.equal(zeroEligibleReadiness.plannedSafeToExecute, false);
const importStep = import1.runGmailSalesPublicBusinessLeadImportOnce();
assert.equal(importStep.status, 'pass');
assert.equal(importStep.attemptedImportCount, 11);
assert.equal(importStep.succeededImportCount, 11);
assert.equal(importStep.failedImportCount, 0);
assert.equal(importStep.readBackMatchedCount, 11);
assert.equal(importStep.importedByBasis.pending_review, 11);
assert.equal(importStep.importedByRuleId.no_api_public_business_lead_import_v1, 11);
assert.equal(importStep.readyInventoryCountBeforeImport, 30);
assert.equal(importStep.readyInventoryCountAfterImport, 30);
assert.equal(importStep.shortfallToExact30BeforeImport, 21);
assert.equal(importStep.currentEligibleCountAfterImport, 9);
assert.equal(importStep.shortfallToExact30AfterImport, 21);
assert.equal(importStep.exact30SatisfiedAfterImport, false);
assert.equal(importStep.manifestReady, false);
...
assert.equal(importSourceAfter.filter((item) => String(item.row.deterministicRuleId || '') === 'no_api_public_business_lead_import_v1').length, 11);
assert.equal(importSourceAfter.filter((item) => String(item.row.contactBasisType || '') === 'valid_business_contact_exception').length, 30);
assert.equal(importSourceAfter.filter((item) => String(item.row.deterministicRuleId || '') === 'no_api_public_business_lead_import_v1' && String(item.row.contactBasisType || '') === '').length, 11);
const importBuilderBody = code.slice(code.indexOf('function buildGmailSalesPublicBusinessLeadImportSourceRow_'), code.indexOf('function inspectGmailSalesNoApiRecoveryBlockerDrilldown'));
assert.equal(importBuilderBody.includes("contactBasisType: 'valid_business_contact_exception'"), false);
assert.equal(importBuilderBody.includes('APPROVED_SEND_MANIFEST_JSON'), false);
assert.equal(importBuilderBody.includes('UrlFetchApp'), false);
...
assert.equal(importSheetAfter.filter((item) => String(item.row.importStatus || '') === 'imported').length, 30);
const importNoApiStatus = import1.inspectGmailSalesNoApiRecoveryStatus({ skipLog: true });
assert.equal(importNoApiStatus.publicBusinessLeadImportEligibleCount, 0);
assert.equal(importNoApiStatus.publicBusinessLeadImportProjectedReadyInventory, 9);
assert.equal(importNoApiStatus.publicBusinessLeadImportShortfallToExact30, 21);
const importUsage = import1.inspectGmailSalesRecoveryUsageLedger();
assert.equal(importUsage.publicBusinessLeadImportEligibleCount, 0);
assert.equal(importUsage.publicBusinessLeadImportProjectedReadyInventory, 9);
assert.equal(importUsage.publicBusinessLeadImportShortfallToExact30, 21);
...
assert.equal(approvalMissingReadiness.operatorRecommendedNextFunction, 'inspectGmailSalesExplicitBatchApprovalPacket');
assert.equal(approvalMissingReadiness.operatorRecommendedNextFunctionReason, 'approval_required_manifest_ready');
assert.equal(approvalMissingReadiness.operatorShouldRunSafeStepNow, false);
const approvalPacket = manifestPending.inspectGmailSalesExplicitBatchApprovalPacket_({ skipLog: true });
assert.equal(approvalPacket.event, 'gmail_sales_explicit_batch_approval_packet');
assert.equal(approvalPacket.mode, 'read_only');
assert.equal(approvalPacket.status, 'pass');
assert.equal(approvalPacket.approvalPacketReady, true);
assert.equal(approvalPacket.manifestReady, true);
assert.equal(approvalPacket.manifestCount, 30);
assert.equal(approvalPacket.manifestUniqueCount, 30);
assert.equal(approvalPacket.manifestDuplicateCount, 0);
assert.equal(approvalPacket.manifestTargetDateMatched, true);
assert.equal(approvalPacket.manifestStale, false);
assert.equal(approvalPacket.currentManifestMaxSendCount, 30);
assert.equal(Boolean(approvalPacket.requiredApprovedBatchId), true);
assert.equal(Boolean(approvalPacket.requiredApprovedBatchChecksum), true);
assert.equal(Boolean(approvalPacket.suggestedApprovalExpiresAt), true);
assert.equal(approvalPacket.suggestedApprovalExpiresAt.startsWith(`${approvalPacket.targetDate}T23:59:59`), true);
assert.equal(approvalPacket.operatorRecommendedNextFunction, 'runGmailSalesExplicitBatchApprovalOnce');
assert.equal(approvalPacket.plannedExpectedApiClass, 'none');
assert.equal(approvalPacket.plannedExpectedWriteClass, 'script_properties_approval_only');
assert.equal(approvalPacket.gmailSendExecuted, false);
assert.equal(approvalPacket.gmailDraftCreated, false);
assert.equal(approvalPacket.triggerChanged, false);
assert.equal(approvalPacket.aiApiCalled, false);
assert.equal(approvalPacket.urlFetchExecuted, false);
...
manifestPending.__props.APPROVED_BATCH_ID = '';
manifestPending.__props.APPROVED_BATCH_CHECKSUM = '';
manifestPending.__props.APPROVAL_EXPIRES_AT = '';
const approvalWriteCountBefore = manifestPending.__state.propertyWriteCount;
const explicitApprovalStep = manifestPending.runGmailSalesExplicitBatchApprovalOnce();
assert.equal(explicitApprovalStep.event, 'gmail_sales_explicit_batch_approval');
assert.equal(explicitApprovalStep.mode, 'safe_step');
assert.equal(explicitApprovalStep.status, 'pass');
assert.equal(explicitApprovalStep.scriptPropertiesUpdated, true);
assert.equal(manifestPending.__state.propertyWriteCount - approvalWriteCountBefore, 3);
assert.equal(explicitApprovalStep.approvedBatchId, approvalPacket.requiredApprovedBatchId);
assert.equal(explicitApprovalStep.approvedBatchChecksum, approvalPacket.requiredApprovedBatchChecksum);
assert.equal(explicitApprovalStep.approvalExpiresAt, approvalPacket.suggestedApprovalExpiresAt);
assert.equal(explicitApprovalStep.approvalReadBackMatched, true);
assert.equal(explicitApprovalStep.approvalPresent, true);
assert.equal(explicitApprovalStep.approvalTargetDateMatched, true);
assert.equal(explicitApprovalStep.approvalManifestDigestMatched, true);
assert.equal(explicitApprovalStep.approvalOperatorConfirmed, true);
assert.equal(explicitApprovalStep.sendReadinessAfterApproval, true);
assert.equal(explicitApprovalStep.sendBlockedReasonAfterApproval, '');
assert.equal(explicitApprovalStep.operatorRecommendedNextFunction, 'runGmailSalesDailyAutomationTrigger');
assert.equal(explicitApprovalStep.operatorRecommendedNextFunctionReason, 'exact30_manifest_approved_send_ready');
assert.equal(explicitApprovalStep.operatorShouldRunSafeStepNow, true);
assert.equal(explicitApprovalStep.nextAction, 'daily_send_ready');
assert.equal(explicitApprovalStep.gmailSendExecuted, false);
assert.equal(explicitApprovalStep.gmailDraftCreated, false);
assert.equal(explicitApprovalStep.triggerChanged, false);
assert.equal(explicitApprovalStep.aiApiCalled, false);
assert.equal(explicitApprovalStep.urlFetchExecuted, false);
assert.equal(manifestPending.__state.gmailSendCount, 0);
assert.equal(manifestPending.__state.draftCreateCount, 0);
assert.equal(manifestPending.__state.triggerCreateCount, 0);
assert.equal(manifestPending.__state.urlFetchCount, 0);
...
const approvalPacketNoManifest = createContext();
installManifestPendingFixture(approvalPacketNoManifest, { importedRows: 19 });
const noManifestPacket = approvalPacketNoManifest.inspectGmailSalesExplicitBatchApprovalPacket_({ skipLog: true });
assert.equal(noManifestPacket.approvalPacketReady, false);
assert.equal(noManifestPacket.status, 'blocked');

const approvalPacketShort = createContext();
installManifestPendingFixture(approvalPacketShort, { rowCount: 29 });
const shortPacket = approvalPacketShort.inspectGmailSalesExplicitBatchApprovalPacket_({ skipLog: true });
assert.equal(shortPacket.approvalPacketReady, false);
assert.equal(shortPacket.status, 'blocked');

const approvalPacketDuplicate = createContext();
installManifestPendingFixture(approvalPacketDuplicate, { duplicateEmail: true });
const duplicatePacket = approvalPacketDuplicate.inspectGmailSalesExplicitBatchApprovalPacket_({ skipLog: true });
assert.equal(duplicatePacket.approvalPacketReady, false);
assert.equal(duplicatePacket.status, 'blocked');

const approvalPacketTargetMismatch = createContext();
installManifestPendingFixture(approvalPacketTargetMismatch, { importedRows: 19 });
approvalPacketTargetMismatch.runGmailSalesManifestBuildOnce();
const mismatchedManifest = JSON.parse(approvalPacketTargetMismatch.__props.APPROVED_SEND_MANIFEST_JSON);
mismatchedManifest.targetDate = '2099-01-01';
approvalPacketTargetMismatch.__props.APPROVED_SEND_MANIFEST_JSON = JSON.stringify(mismatchedManifest);
const mismatchPacket = approvalPacketTargetMismatch.inspectGmailSalesExplicitBatchApprovalPacket_({ skipLog: true });
assert.equal(mismatchPacket.approvalPacketReady, false);
assert.equal(mismatchPacket.approvalPacketBlockedReason, 'manifest_target_date_mismatch');
